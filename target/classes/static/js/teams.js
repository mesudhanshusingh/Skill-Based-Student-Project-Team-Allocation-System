// Direct Strictly Allocated Teams View JavaScript

document.addEventListener('DOMContentLoaded', async () => {
    await loadAllocatedTeams();
});

// Load only allocated teams from REST API
async function loadAllocatedTeams() {
    ...
    try {
        ...
        // Filter ONLY projects that have allocated teams
        const allocatedGroups = (data || []).filter(g => g.teams && g.teams.length > 0);

        renderAllocatedTeamsGrid(allocatedGroups, container);
    } catch (error) {
        showAlert('alertContainer', 'Failed to load allocated teams: ' + error.message);
...
// Render allocated teams grid
function renderAllocatedTeamsGrid(allocatedGroups, container) {
    container.innerHTML = '';

    if (!allocatedGroups || allocatedGroups.length === 0) {
        container.innerHTML = `
    ...
    allocatedGroups.forEach(group => {
        const requiredSkillsHtml = (group.requiredSkills || '')
            .split(',')
            ...
            card.innerHTML = `
                <div>
                    <div class="team-header" style="display: flex; justify-content: space-between; align-items: flex-start;">
                        <div>
                            ...
                            <div style="margin-top: 0.25rem;">
                                <span class="badge badge-warning">Posted by: ${escapeHtml(group.createdByName || 'Student Lead')}</span>
                            </div>
                        </div>
                    ...
                    <div style="margin-bottom: 0.75rem; font-size: 0.8rem; color: var(--text-secondary);">
                        <strong>Project Requirements:</strong> ${requiredSkillsHtml}
                    </div>
                ...
                <div>
                    <div class="coverage-box">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <span class="badge ${team.skillCoverage >= 100 ? 'badge-success' : 'badge-warning'}" style="font-size: 0.85rem;">
                                ${team.skillCoverage}%
                            </span>
                        </div>
                        ${missingSkillsHtml}
...
// Reset teams for a project
async function resetTeamsForProject(projectId) {
    if (!confirm('Are you sure you want to reset/delete this team? Members will be set back to UNALLOCATED.')) {
        return;
    }
    ...
    try {
        ...
        showAlert('alertContainer', 'Failed to reset team: ' + error.message);
    }
}
